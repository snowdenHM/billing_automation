#!/usr/bin/env python3
"""
Zoho Journal Bills - Consolidation Object Creation Script
Creates consolidated products for existing Zoho journal bills with multiple line items.

Usage: python create_zoho_journal_consolidate.py
"""
...
from apps.module.zoho.models import (
    JournalZohoBill, JournalZohoProduct, JournalZohoConsolidatedProduct
)

def create_consolidated_journal_products():
    """Create consolidated products for Zoho journal bills with multiple line items"""
    print("🚀 Starting consolidated product creation for existing Zoho journal bills...")
    print("=" * 70)

    # Find all journal bills with multiple products (2 or more)
    bills_with_multiple_products = JournalZohoBill.objects.filter(
    ...
    print(f"📊 Found {bills_with_multiple_products.count()} journal bills with multiple products")
    print("-" * 70)
    ...
    for idx, bill in enumerate(bills_with_multiple_products, 1):
        ...
        try:
            ...
            existing_consolidated = JournalZohoConsolidatedProduct.objects.filter(
                zohoBill=bill
            ).first()
            ...
            for product in products:
                ...
                item_details.append(
                    f"• {product.item_details} ({debit_credit.title()}: ₹{amount})"
                )

            consolidated_details = f"Consolidated {product_count} journal entries:\n" + "\n".join(item_details)

            # Determine consolidated debit_or_credit (use most common or first one)
            debit_credits = [getattr(p, 'debit_or_credit', 'debit') for p in products]
            most_common_debit_credit = max(set(debit_credits), key=debit_credits.count)
            ...
            if existing_consolidated:
                ...
                existing_consolidated.consolidation_notes = f'Updated script run - {product_count} journal entries consolidated'
                ...
                JournalZohoConsolidatedProduct.objects.create(
                    ...
                    consolidation_notes=f'Script created - {product_count} journal entries consolidated'
                )
    ...
    if created_count > 0 or updated_count > 0:
        print("✅ Consolidated products are ready for Zoho journal bills!")
...
def update_consolidate_flags(created_count, updated_count):
    ...
    bills_updated = JournalZohoBill.objects.filter(
...
if __name__ == "__main__":
    print("🎯 Zoho Journal Bills - Consolidated Product Creation")
    print("This script will create consolidated products for bills with multiple line items.")
    print()

    try:
        create_consolidated_journal_products()
