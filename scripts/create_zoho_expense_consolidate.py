#!/usr/bin/env python3
"""
Zoho Expense Bills - Consolidation Object Creation Script
Creates consolidated products for existing Zoho expense bills with multiple line items.

Usage: python create_zoho_expense_consolidate.py
"""
...
from apps.module.zoho.models import (
    ExpenseZohoBill, ExpenseZohoProduct, ExpenseZohoConsolidatedProduct
)

def create_consolidated_expense_products():
    """Create consolidated products for Zoho expense bills with multiple line items"""
    print("🚀 Starting consolidated product creation for existing Zoho expense bills...")
    print("=" * 70)

    # Find all expense bills with multiple products (2 or more)
    bills_with_multiple_products = ExpenseZohoBill.objects.filter(
    ...
    for idx, bill in enumerate(bills_with_multiple_products, 1):
        ...
        try:
            ...
            existing_consolidated = ExpenseZohoConsolidatedProduct.objects.filter(
                zohoBill=bill
            ).first()
            ...
            for product in products:
                ...
                item_details.append(
                    f"• {product.item_details} (Amount: ₹{amount})"
                )
    ...
    if created_count > 0 or updated_count > 0:
        print("✅ Consolidated products are ready for Zoho expense bills!")
...
def update_consolidate_flags(created_count, updated_count):
    ...
    bills_updated = ExpenseZohoBill.objects.filter(
...
if __name__ == "__main__":
    print("🎯 Zoho Expense Bills - Consolidated Product Creation")
