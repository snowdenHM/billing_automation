...
def process_zoho_vendor_bill_analysis(bill_id, organization_id, **kwargs):
    ...
    def get_analyze_function():
        from .vendor_views import analyze_vendor_bill_with_openai, create_vendor_zoho_objects_from_analysis, check_duplicate_bill
    ...
    try:
        ...
        try:
            ...
            logger.error(f"AI analysis failed for Zoho vendor bill {bill_id}: {str(e)}")
        ...
        try:
            ...
            if duplicate_result:
                ...
                logger.info(f"Duplicate check completed for Zoho vendor bill {bill_id}: {is_duplicate}")
                
        except Exception as e:
            logger.error(f"Duplicate check failed for Zoho vendor bill {bill_id}: {str(e)}")
        ...
        logger.info(f"Background processing completed for Zoho vendor bill {bill_id}")
        return f"Successfully processed Zoho vendor bill {bill_id}"
        
    except VendorBill.DoesNotExist:
        logger.error(f"Zoho vendor bill {bill_id} not found")
        raise
    except Exception as e:
        logger.error(f"Background processing failed for Zoho vendor bill {bill_id}: {str(e)}")
        try:
            bill = VendorBill.objects.get(id=bill_id)
            bill.is_processing = False
            bill.processing_error = f"Processing failed: {str(e)}"
            bill.save(update_fields=['is_processing', 'processing_error'])
        except:
            pass
        raise
...
def process_zoho_expense_bill_analysis(bill_id, organization_id, **kwargs):
    ...
    try:
        ...
        logger.info(f"Starting background processing for Zoho expense bill {bill_id}")
        ...
        try:
            ...
            logger.error(f"AI analysis failed for Zoho expense bill {bill_id}: {str(e)}")
            bill.is_processing = False
            bill.processing_error = f"AI analysis failed: {str(e)}"
            bill.save(update_fields=['is_processing', 'processing_error'])
            raise
        ...
        logger.info(f"Background processing completed for Zoho expense bill {bill_id}")
        return f"Successfully processed Zoho expense bill {bill_id}"
        
    except ExpenseBill.DoesNotExist:
        logger.error(f"Zoho expense bill {bill_id} not found")
        raise
    except Exception as e:
        logger.error(f"Background processing failed for Zoho expense bill {bill_id}: {str(e)}")
        try:
            bill = ExpenseBill.objects.get(id=bill_id)
            bill.is_processing = False
            bill.processing_error = f"Processing failed: {str(e)}"
...
def process_zoho_journal_bill_analysis(bill_id, organization_id, **kwargs):
    ...
    try:
        ...
        logger.info(f"Starting background processing for Zoho journal bill {bill_id}")
        ...
        try:
            ...
            logger.error(f"AI analysis failed for Zoho journal bill {bill_id}: {str(e)}")
            bill.is_processing = False
            bill.processing_error = f"AI analysis failed: {str(e)}"
            bill.save(update_fields=['is_processing', 'processing_error'])
            raise
        ...
        logger.info(f"Background processing completed for Zoho journal bill {bill_id}")
        return f"Successfully processed Zoho journal bill {bill_id}"
        
    except JournalBill.DoesNotExist:
        logger.error(f"Zoho journal bill {bill_id} not found")
        raise
    except Exception as e:
        logger.error(f"Background processing failed for Zoho journal bill {bill_id}: {str(e)}")
        try:
            bill = JournalBill.objects.get(id=bill_id)
            bill.is_processing = False
            bill.processing_error = f"Processing failed: {str(e)}"
