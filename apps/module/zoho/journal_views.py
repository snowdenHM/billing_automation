...
def check_duplicate_journal_bill(bill, organization):
    """
    Check if a journal bill is a duplicate based on invoice number and vendor information.
    Returns tuple (is_duplicate, duplicate_bills, similarity_score)
    """
    logger.info(f"Checking for duplicates of journal bill {bill.id} in organization {organization.id}")
    ...
    potential_duplicates = JournalBill.objects.filter(
    ...
    for other_bill in potential_duplicates:
        ...
        if current_vendor_name and other_vendor_name:
            vendor_similarity = _calculate_string_similarity_journal(current_vendor_name.lower(), other_vendor_name.lower())
    ...
    logger.info(f"Journal duplicate check complete. Found {len(duplicate_bills)} potential duplicates with max similarity {max_similarity}")

    return is_duplicate, duplicate_bills, max_similarity


def _calculate_string_similarity_journal(str1, str2):
    ...
    try:
        ...
        if len(created_products) > 1:
            try:
                ...
                JournalZohoConsolidatedProduct.objects.filter(zohoBill=zoho_bill).delete()
                ...
                consolidated_details = f'Consolidated {items_count} journal entries:\n' + '\n'.join(item_details)
                ...
                consolidated_product = JournalZohoConsolidatedProduct.objects.create(
                    ...
                    consolidation_notes=f'Auto-created during analysis for {items_count} journal entries'
                )

                logger.info(f"✅ Auto-created consolidated journal product for bill {zoho_bill.id} with {items_count} entries (₹{total_amount})")

            except Exception as e:
                logger.error(f"❌ Error creating consolidated journal product for bill {zoho_bill.id}: {str(e)}")
                # Don't raise - consolidated product creation failure shouldn't break the main flow
        else:
            logger.info(f"ℹ️ Skipping consolidated journal product creation - bill has only {len(created_products)} item(s)")
...
def journal_bill_upload_view(request, org_id):
    ...
    try:
        ...
        for i, uploaded_file in enumerate(files):
                ...
                similar_files = JournalBill.objects.filter(
                    organization=organization,
                    file__isnull=False
        ...
        for i, bill in enumerate(created_bills):
            ...
            if bill.status == 'Draft':
                try:
                    ...
                    is_duplicate, duplicate_bills, max_similarity = check_duplicate_journal_bill(bill, organization)
                    ...
                    if is_duplicate:
                        ...
                        analysis_result.update({
                            ...
                            "warning_message": f"⚠️ DUPLICATE DETECTED: Journal Bill '{bill.billmunshiName}' appears to be {round(max_similarity, 1)}% similar to {len(duplicate_bills)} existing bill(s)."
                        })
        ...
        logger.info(f"Successfully processed {len(files)} files and created {len(created_bills)} journal bills")

        response_data = {
            'message': f'Successfully uploaded {len(files)} file(s) and created {len(created_bills)} journal bill(s)',
        ...
        if upload_warnings or all_duplicate_warnings:
            ...
            if all_duplicate_warnings:
                warning_messages.append(f"🔍 CONTENT WARNING: {len(all_duplicate_warnings)} duplicate(s) detected after analyzing journal bill content")
            ...
            logger.warning(f"Journal bills - Total warnings generated: {warnings_count} (Upload: {len(upload_warnings)}, Content: {len(all_duplicate_warnings)})")
...
def journal_bill_analyze_view(request, org_id, bill_id):
    ...
    try:
        ...
        response_data = {
            "detail": "Journal bill analyzed successfully",
        ...
        if is_duplicate:
            ...
            response_data.update({
                ...
                "max_similarity": round(max_similarity, 2),
                "duplicate_bills": duplicate_warnings,
                "warning_message": f"⚠️ DUPLICATE DETECTED: Found {len(duplicate_bills)} similar journal bill(s) in your organization. "
            ...
            logger.warning(f"Duplicate journal bill detected for {bill.billmunshiName} - {len(duplicate_bills)} similar bills found")
...
def journal_bill_sync_view(request, org_id, bill_id):
    ...
    try:
        ...
        # 🔄 Check consolidation flag and use appropriate data source for line items
        ...
        if not hasattr(zoho_bill, 'consolidate') or not zoho_bill.consolidate:
            # ✅ USE INDIVIDUAL JOURNAL PRODUCTS (Original logic)
            logger.info(f"[DEBUG] journal_bill_sync_view - Using individual journal products for bill {bill_id}")
