from __future__ import annotations
import base64
import json
import os
from io import BytesIO
from typing import List, Dict, Any, Tuple

from django.conf import settings
from django.core.files.base import ContentFile
from django.db import transaction
from django.shortcuts import get_object_or_404

import requests
from pdf2image import convert_from_bytes

from rest_framework import permissions, status
from rest_framework.generics import ListCreateAPIView, RetrieveAPIView
from rest_framework.response import Response
from rest_framework.views import APIView
from drf_spectacular.utils import extend_schema, OpenApiParameter

from apps.organizations.models import Organization
from apps.module.zoho.models import (
    VendorBill,
    VendorZohoBill,
    VendorZohoProduct,
    ZohoVendor,
    ZohoTdsTcs,
    ZohoChartOfAccount,
    ZohoTaxes,
    ZohoCredentials,
)
from apps.module.zoho.permissions import IsOrgAdminOrOrgAPIKey, ModuleZohoEnabled
from apps.module.zoho.serializers.vendor_bills import (
    VendorBillUploadSerializer,
    VendorBillSerializer,
    VendorBillDetailSerializer,
    VendorZohoBillSerializer,
    VendorZohoProductSerializer,
    VendorBillVerifySerializer,
    UploadResultSerializer,
)
from apps.module.zoho.serializers.base import SyncResultSerializer  # ← use shared serializer


# -------------------- Helpers --------------------

def _get_org(org_id):
    """
    Get organization by ID, supporting UUID.
    """
    return get_object_or_404(Organization, id=org_id)


def _get_org_creds(org: Organization) -> ZohoCredentials:
    return get_object_or_404(ZohoCredentials, organization=org)


def _openai_analyse_images(image_bytes_list: List[bytes]) -> Dict[str, Any]:
    """
    Sends one or many images to OpenAI to extract invoice data.
    If OPENAI_API_KEY is missing or the API fails, raises an exception.
    The model receives all images (for multipage invoices) in a single prompt.
    """
    api_key = getattr(settings, "OPENAI_API_KEY", None)
    if not api_key:
        raise RuntimeError("OPENAI_API_KEY not configured")

    from openai import OpenAI
    client = OpenAI(api_key=api_key)

    invoice_schema = {
        "$schema": "http://json-schema.org/draft/2020-12/schema",
        "title": "Invoice",
        "type": "object",
        "properties": {
            "invoiceNumber": {"type": "string"},
            "dateIssued": {"type": "string"},
            "dueDate": {"type": "string"},
            "from": {"type": "object", "properties": {"name": {"type": "string"}, "address": {"type": "string"}}},
            "to": {"type": "object", "properties": {"name": {"type": "string"}, "address": {"type": "string"}}},
            "items": {
                "type": "array",
                "items": {
                    "type": "object",
                    "properties": {"description": {"type": "string"}, "quantity": {"type": "number"}, "price": {"type": "number"}},
                },
            },
            "total": {"type": "number"},
            "igst": {"type": "number"},
            "cgst": {"type": "number"},
            "sgst": {"type": "number"},
        },
    }

    content = [{"type": "text", "text": "Provide a JSON for this invoice using this JSON Schema: " + json.dumps(invoice_schema)}]
    for img in image_bytes_list:
        b64 = base64.b64encode(img).decode("utf-8")
        content.append({"type": "image_url", "image_url": {"url": f"data:image/jpeg;base64,{b64}"}})

    prompt = [{"role": "user", "content": content}]
    resp = client.chat.completions.create(
        model="gpt-4o",
        response_format={"type": "json_object"},
        messages=prompt,
        max_tokens=1200,
    )
    return json.loads(resp.choices[0].message.content)


def _pdf_to_all_pages_jpegs(data: bytes) -> List[bytes]:
    images = convert_from_bytes(data)
    out: List[bytes] = []
    for im in images:
        buf = BytesIO()
        im.save(buf, format="JPEG")
        out.append(buf.getvalue())
    return out


def _file_to_pages(request_file) -> Tuple[List[bytes], bool]:
    name = getattr(request_file, "name", "").lower()
    data = request_file.read()
    request_file.seek(0)

    if name.endswith(".pdf"):
        return _pdf_to_all_pages_jpegs(data), True
    return [data], False


def _match_vendor_by_name(org: Organization, company_name: str) -> ZohoVendor | None:
    if not company_name:
        return None
    try:
        from django.db.models.functions import Lower
        return (
            ZohoVendor.objects.filter(organization=org)
            .annotate(lower_name=Lower("companyName"))
            .filter(lower_name=company_name.strip().lower())
            .first()
        )
    except Exception:
        return None


def _refresh_zoho_access_token(creds: ZohoCredentials) -> str | None:
    if not creds.refreshToken:
        return None
    url = (
        "https://accounts.zoho.in/oauth/v2/token"
        f"?refresh_token={creds.refreshToken}&client_id={creds.clientId}"
        f"&client_secret={creds.clientSecret}&grant_type=refresh_token"
    )
    r = requests.post(url, timeout=30)
    if r.status_code == 200:
        at = r.json().get("access_token")
        if at:
            creds.accessToken = at
            creds.save(update_fields=["accessToken"])
            return at
    return None


def _persist_analysis_to_models(org: Organization, bill: VendorBill, analysed_json: Dict[str, Any]) -> None:
    bill.analysed_data = analysed_json
    bill.status = "Analysed"
    bill.process = True
    bill.save(update_fields=["analysed_data", "status", "process"])

    invoice_no = analysed_json.get("invoiceNumber", "")
    date_issued = analysed_json.get("dateIssued") or None
    igst = analysed_json.get("igst", 0)
    cgst = analysed_json.get("cgst", 0)
    sgst = analysed_json.get("sgst", 0)
    total = analysed_json.get("total", 0)

    vendor_name = (analysed_json.get("from") or {}).get("name", "")
    vendor = _match_vendor_by_name(org, vendor_name)

    zbill = VendorZohoBill.objects.create(
        organization=org,
        selectBill=bill,
        vendor=vendor,
        bill_no=invoice_no,
        bill_date=date_issued,
        igst=igst or 0,
        cgst=cgst or 0,
        sgst=sgst or 0,
        total=total or 0,
        note="Analysed Bill",
    )

    for item in (analysed_json.get("items") or []):
        desc = item.get("description", "")
        qty = item.get("quantity", 0)
        price = item.get("price", 0)
        VendorZohoProduct.objects.create(
            organization=org,
            zohoBill=zbill,
            item_name=desc,
            item_details=desc,
            rate=str(price),
            quantity=str(qty),
            amount=str(price * qty) if isinstance(price, (int, float)) and isinstance(qty, (int, float)) else "",
        )


# -------------------- Views --------------------

@extend_schema(
    tags=["Zoho Vendor Bills"],
    parameters=[
        OpenApiParameter(name="status", required=False, type=str, location=OpenApiParameter.QUERY),
    ],
    request=VendorBillUploadSerializer,
    responses={200: UploadResultSerializer, 201: UploadResultSerializer},
)
class VendorBillListCreateView(ListCreateAPIView):
    permission_classes = [permissions.IsAuthenticated | IsOrgAdminOrOrgAPIKey, ModuleZohoEnabled]
    serializer_class = VendorBillSerializer
    queryset = VendorBill.objects.none()

    def get_queryset(self):
        if getattr(self, "swagger_fake_view", False):
            return VendorBill.objects.none()
        org_id = self.kwargs["org_id"]
        qs = VendorBill.objects.filter(organization_id=org_id).order_by("-created_at")
        status_param = self.request.query_params.get("status")
        if status_param:
            qs = qs.filter(status=status_param)
        return qs

    def get_serializer_class(self):
        if self.request.method == "POST":
            return VendorBillUploadSerializer
        return VendorBillSerializer

    @transaction.atomic
    def create(self, request, *args, **kwargs):
        upload_ser = VendorBillUploadSerializer(data=request.data)
        upload_ser.is_valid(raise_exception=True)
        org = _get_org(kwargs["org_id"])
        file = upload_ser.validated_data["file"]
        file_type = upload_ser.validated_data.get("fileType") or "Single Invoice/File"

        pages, is_pdf = _file_to_pages(file)

        created_bills: List[VendorBill] = []

        if file_type == "Single Invoice/File":
            bill = VendorBill.objects.create(
                organization=org,
                file=file,
                fileType=file_type,
                status="Draft",
            )
            try:
                analysed_json = _openai_analyse_images(pages)  # all pages
                _persist_analysis_to_models(org, bill, analysed_json)
            except Exception:
                pass
            created_bills.append(bill)
        else:
            if not is_pdf:
                bill = VendorBill.objects.create(
                    organization=org,
                    file=file,
                    fileType=file_type,
                    status="Draft",
                )
                try:
                    analysed_json = _openai_analyse_images(pages)
                    _persist_analysis_to_models(org, bill, analysed_json)
                except Exception:
                    pass
                created_bills.append(bill)
            else:
                for idx, jpeg_bytes in enumerate(pages, start=1):
                    page_name = f"page-{idx}.jpg"
                    bill = VendorBill.objects.create(
                        organization=org,
                        file=ContentFile(jpeg_bytes, name=page_name),
                        fileType=file_type,
                        status="Draft",
                    )
                    try:
                        analysed_json = _openai_analyse_images([jpeg_bytes])
                        _persist_analysis_to_models(org, bill, analysed_json)
                    except Exception:
                        pass
                    created_bills.append(bill)

        out = UploadResultSerializer({
            "created": len(created_bills),
            "bills": VendorBillSerializer(created_bills, many=True).data,
        }).data
        return Response(out, status=status.HTTP_201_CREATED)


@extend_schema(
    tags=["Zoho Vendor Bills"],
    responses=VendorBillDetailSerializer,
)
class VendorBillDetailView(RetrieveAPIView):
    permission_classes = [permissions.IsAuthenticated | IsOrgAdminOrOrgAPIKey, ModuleZohoEnabled]
    serializer_class = VendorBillDetailSerializer
    queryset = VendorBill.objects.none()

    def retrieve(self, request, *args, **kwargs):
        org_id = kwargs["org_id"]
        bill = get_object_or_404(VendorBill, id=kwargs["bill_id"], organization_id=org_id)
        try:
            zbill = VendorZohoBill.objects.select_related("vendor").prefetch_related("products").get(
                selectBill=bill, organization_id=org_id
            )
            data = {
                "bill": VendorBillSerializer(bill).data,
                "analysed": VendorZohoBillSerializer(zbill).data,
            }
        except VendorZohoBill.DoesNotExist:
            data = {"bill": VendorBillSerializer(bill).data, "analysed": None}
        return Response(data)


@extend_schema(
    tags=["Zoho Vendor Bills"],
    request=VendorBillVerifySerializer,
    responses=VendorZohoBillSerializer,
)
class VendorBillVerifyView(APIView):
    permission_classes = [permissions.IsAuthenticated | IsOrgAdminOrOrgAPIKey, ModuleZohoEnabled]
    serializer_class = VendorBillVerifySerializer

    @transaction.atomic
    def post(self, request, org_id: int, bill_id, *args, **kwargs):
        bill = get_object_or_404(VendorBill, id=bill_id, organization_id=org_id)
        zbill = get_object_or_404(VendorZohoBill, selectBill=bill, organization_id=org_id)

        ser = VendorBillVerifySerializer(data=request.data)
        ser.is_valid(raise_exception=True)
        data = ser.validated_data

        for field in ["vendor", "note", "bill_no", "bill_date", "cgst", "sgst", "igst"]:
            if field in data:
                setattr(zbill, field, data[field])

        tax_type = data.get("tax_type", "No")
        tds_tcs = data.get("tds_tcs_id")
        if tax_type in ("TDS", "TCS"):
            zbill.is_tax = tax_type
            zbill.tds_tcs_id = tds_tcs
        else:
            zbill.is_tax = None
            zbill.tds_tcs_id = None

        zbill.save()

        updates = {str(p["id"]): p for p in data.get("products", [])}
        if updates:
            for prod in VendorZohoProduct.objects.filter(zohoBill=zbill):
                p = updates.get(str(prod.id))
                if not p:
                    continue
                if "chart_of_accounts" in p:
                    prod.chart_of_accounts = p["chart_of_accounts"]
                if "taxes" in p:
                    prod.taxes = p["taxes"]
                if "reverse_charge_tax_id" in p:
                    prod.reverse_charge_tax_id = p["reverse_charge_tax_id"]
                if "itc_eligibility" in p:
                    prod.itc_eligibility = p["itc_eligibility"]
                prod.save()

        bill.status = "Verified"
        bill.save(update_fields=["status"])

        return Response(VendorZohoBillSerializer(zbill).data, status=status.HTTP_200_OK)


@extend_schema(
    tags=["Zoho Vendor Bills"],
    request=None,
    responses=SyncResultSerializer,
)
class VendorBillSyncView(APIView):
    permission_classes = [permissions.IsAuthenticated | IsOrgAdminOrOrgAPIKey, ModuleZohoEnabled]
    serializer_class = SyncResultSerializer

    def post(self, request, org_id: int, bill_id, *args, **kwargs):
        org = _get_org(org_id)
        bill = get_object_or_404(VendorBill, id=bill_id, organization=org)
        if bill.status not in ("Verified", "Analysed"):
            return Response({"synced": False, "message": "Bill is not verified/analysed."}, status=400)

        try:
            zbill = VendorZohoBill.objects.select_related("vendor", "tds_tcs_id").get(
                selectBill=bill, organization=org
            )
        except VendorZohoBill.DoesNotExist:
            return Response({"synced": False, "message": "Analysed bill not found."}, status=404)

        products = list(VendorZohoProduct.objects.select_related("chart_of_accounts", "taxes").filter(zohoBill=zbill))

        if not zbill.vendor:
            return Response({"synced": False, "message": "Vendor is required to sync."}, status=400)

        vendor_contact_id = zbill.vendor.contactId
        bill_date_str = zbill.bill_date.isoformat() if zbill.bill_date else None
        payload: Dict[str, Any] = {
            "vendor_id": vendor_contact_id,
            "bill_number": zbill.bill_no or "",
            "gst_no": zbill.vendor.gstNo or "",
            "date": bill_date_str,
            "line_items": [],
        }

        if zbill.is_tax in ("TDS", "TCS") and zbill.tds_tcs_id:
            if zbill.is_tax == "TDS":
                payload["tds_tax_id"] = str(zbill.tds_tcs_id.taxId)
            else:
                payload["tcs_tax_id"] = str(zbill.tds_tcs_id.taxId)

        for item in products:
            line: Dict[str, Any] = {
                "account_id": str(item.chart_of_accounts.accountId) if item.chart_of_accounts else "",
                "rate": item.rate or "0",
                "quantity": float(item.quantity or 0),
                "discount": 0.0,
                "itc_eligibility": item.itc_eligibility or "eligible",
            }
            if item.reverse_charge_tax_id and item.taxes:
                line["reverse_charge_tax_id"] = str(item.taxes.taxId)
            elif item.taxes:
                line["tax_id"] = str(item.taxes.taxId)

            payload["line_items"].append(line)

        creds = _get_org_creds(org)
        url = f"https://www.zohoapis.in/books/v3/bills?organization_id={creds.organisationId}"
        headers = {"Authorization": f"Zoho-oauthtoken {creds.accessToken}"}
        r = requests.post(url, headers=headers, data=json.dumps(payload), timeout=60)

        if r.status_code == 401:
            new_at = _refresh_zoho_access_token(creds)
            if new_at:
                headers["Authorization"] = f"Zoho-oauthtoken {new_at}"
                r = requests.post(url, headers=headers, data=json.dumps(payload), timeout=60)

        if r.status_code in (200, 201):
            bill.status = "Synced"
            bill.save(update_fields=["status"])
            return Response({"synced": True, "message": "Bill synced successfully."})

        try:
            err = r.json().get("message", r.text)
        except Exception:
            err = r.text
        return Response({"synced": False, "message": f"Zoho error: {err}"}, status=502)
