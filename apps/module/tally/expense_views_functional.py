    ...
    try:
        ...
        for bill in created_bills:
            try:
                ...
                job = enqueue_expense_bill_processing(str(bill.id))
                bill.job_id = job.id
                ...
                logger.info(f"Queued expense bill {bill.bill_munshi_name} for background processing")
            except Exception as e:
                logger.error(f"Failed to queue expense bill {bill.bill_munshi_name}: {str(e)}")
                job_results.append({
                    'bill_id': str(bill.id),
                    'bill_name': bill.bill_munshi_name,
                    'job_id': None,
        ...
        response_data = {
            'message': f'Successfully uploaded {len(files)} file(s) and created {len(created_bills)} expense bill(s). Processing started in background.',
...
def expense_bill_processing_status(request, org_id, bill_id):
    ...
    try:
        ...
        return Response({
            ...
            'message': f'Expense bill with ID {bill_id} not found in organization {org_id}.'
        }, status=status.HTTP_404_NOT_FOUND)
    except Exception as e:
        logger.error(f"Error checking expense bill processing status: {str(e)}")
        return Response({
